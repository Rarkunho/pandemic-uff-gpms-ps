...
# Aqui é definido a quantidade de ações por turno. No jogo original é 4.
ACTIONS_PER_TURN = 4
...
class Game:
    def __init__(self):
        ...
        self.turn_actions_remaining = ACTIONS_PER_TURN 
        ...
        self.board = None
        self.infected_cities_this_turn = set()
        
    def reset_infected_cities(self):
        self.infected_cities_this_turn.clear()
    
    def next_turn(self):
        self.reset_infected_cities()
    ...
    def perform_action(self, action, *args, board=None, **kwargs):
        if self.turn_actions_remaining > 0:
            ...
            if action in ['treat_disease', 'find_cure']:
                ...
                if action == 'build_center':
                    success = player.build_center()
                else:
